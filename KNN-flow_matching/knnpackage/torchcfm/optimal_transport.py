...
class OTPlanSampler:
    ...
    def sample_plan(self, x0, x1, replace=True):
        ...
        """
        k = 4
        # First, generate n noises for each data in x_0
        x0 = torch.randn(x1.shape[0], k, x1.shape[1], x1.shape[2], x1.shape[3], device=x1.device, dtype=x1.dtype) # [B, k, C, H, W]

        x1_points = x1.flatten(start_dim=1).to(torch.float16) # [B, D=H*W*C]
        x0_points = x0.flatten(start_dim=2).to(torch.float16)  # [B,k, D=H*W*C]

        # Calculate the distance between x_0 and the corresponding k noises
        distance_points = x1_points.unsqueeze(1) - x0_points # [B, k, D]
        distance = torch.linalg.vector_norm(distance_points, dim=2) # [B, k]

        # Pick the nearest noise for each data in x_0
        _, min_index = torch.min(distance, dim=1) # [B]
        sorted_x0 = torch.gather(x0, 1, min_index.unsqueeze(1).unsqueeze(2).unsqueeze(3).unsqueeze(4).expand(-1, -1, x1.shape[1], x1.shape[2], x1.shape[3]))[:, 0, :, :, :] # [B, C, H, W]

        # print("Distance Before KNN", distance[:, 0].mean())
        # row = torch.arange(x1.shape[0], device=x1.device, dtype=torch.long)
        # print("Distance After KNN", distance[row, min_index].mean())
        # print("Distance Reduction Rate", (distance[:, 0].mean() - distance[row, min_index].mean()) / distance[:, 0].mean())
        return sorted_x0, x1
